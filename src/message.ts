...
export class Message {
		...
		for (const header in headers) {
			...
			if (/^content-type$/i.test(header)) {
				...
				if (Array.isArray(attachment)) {
					...
					this.attach(attachment as MessageAttachment)
				}
	...
	public read(callback: (err: Error | null, buffer: string) => void) {
		let buffer = ''
		const str = this.stream()
		str.on('data', (data) => (buffer += data))
		str.on('end', (err: Error) => callback(err, buffer))
		str.on('error', (err: Error) => callback(err, buffer))
	}
...
class MessageStream extends Stream {
	readable = true
	paused = false
	buffer: Buffer | null = Buffer.alloc(BUFFERSIZE)
	bufferIndex = 0
